...
prefeitos_pb_2012 <- read_csv("dados/prefeitos_eleitos_pb_2012.csv")

#votos_PB <- vote_mun_zone_local(2012, uf = "PB", export = TRUE)
#prefeitos_pb_2012 <- votos_PB %>%
 #filter(DESC_SIT_CAND_TOT == "ELEITO", DESCRICAO_CARGO == "PREFEITO") %>%
  ...
  #distinct()
#write_csv(prefeitos_pb_2012, "dados/prefeitos_eleitos_pb_2012.csv")
...
participacoes_municipio_licitacoes <- participacoes_de_licitacoes %>%
  ...
  left_join(select(prefeitos_pb_2012, prefeito = NOME_CANDIDATO, municipio = NOME_MUNICIPIO))

participacoes_municipio_licitacoes

doacoes_municipio_2012 <- doacoes_2012 %>%
  group_by(nome_doador = `Nome do doador (Receita Federal)`,
           nome_candidato = `Nome candidato`,
           municipio = Municipio) %>%
  summarise(n_doacoes =  n(),
            total_doado = sum(as.numeric(`Valor receita`), na.rm = TRUE))
doacoes_municipio_2012

doacoes_participacoes_prefeito <- doacoes_municipio_2012 %>%
...
d_l_municipios <- doacoes_participacoes_prefeito %>%
  ...
  summarise(total_doado= sum(total_doado),
            valor_recebido_licitacao = sum((valor_recebido)))
...
d_l_empresa <- doacoes_participacoes_prefeito %>%
  ...
  summarise(n_doacoes = sum(n_doacoes),
            n_licitacao_vencedora = sum(n_licitacoes)) %>%
...
participacoes_municipio_licitacoes_indireta <- participacoes_de_licitacoes %>%
  ...
  summarise(n_licitacoes = sum(n_licitacoes),
            n_vencedora = sum(n_vencedora),
            valor_recebido = sum(valor_recebido, na.rm = TRUE)) %>%
  left_join(select(prefeitos_pb_2012, prefeito = NOME_CANDIDATO, partido_sigla = SIGLA_PARTIDO, municipio = NOME_MUNICIPIO))

participacoes_municipio_licitacoes_indireta

doacoes_municipio_2012_indireta <- doacoes_2012 %>%
  group_by(nome_doador = `Nome do doador (Receita Federal)`,
           nome_candidato = `Nome candidato`,
...
doacoes_municipio_2012_indireta%>%
  arrange(desc(total_doado))

doacoes_participacoes_prefeito_indireta <- doacoes_municipio_2012_indireta%>%
